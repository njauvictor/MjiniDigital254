...
export async function generateMetadata({
  ...
  const serviceDescription = description || `${title} - Professional web development services by Mjini Digital, Nairobi's Gold Standard in Web Development`;
  const authorName = author?.name || "Mjini Digital";
  const tagNames = tags.map(tag => tag.name).join(", ");
  const publishedTime = new Date(createdAt).toISOString();
  const modifiedTime = new Date(updatedAt).toISOString();

  return {
    title: `${title} | Mjini Digital | Website Developer in Nairobi`,
...
export default async function OurServices({
  ...
  // If no post found, show a 404 page instead of returning null
  if (!result.post) {
    return (
